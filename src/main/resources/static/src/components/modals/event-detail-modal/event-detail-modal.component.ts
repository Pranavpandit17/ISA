...
export class EventDetailModalComponent implements OnInit {
  ...
  onRegister(): void {
    ...
    // For paid events, open payment modal (same as membership plan switching)
    if (this.isEventPaid()) {
      // Close event detail modal and open payment modal
      this.onClose();
      // Emit event with event data and quantity to parent
      this.openPayment.emit({ event: this.event, quantity: this.quantity });
      return;
    }
  ...
  getEventPrice(): number {
    ...
    // For paid events, choose price based on member type
  ...
  getMaxQuantity(): number {
    ...
    if (!capacity) return 10; // Default max if no capacity limit
