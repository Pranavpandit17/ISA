...
export class BenchResourcesComponent implements OnInit {
  ...
  onArchiveResource(resource: Resource): void {
    ...
    if (confirm(`Are you sure you want to archive this resource posting? Archived resources will be hidden from public view but can be restored later.`)) {
      this.isLoading = true;
      this.apiService.archiveBenchResource(resource.id).subscribe({
        next: () => {
        ...
        error: (error) => {
          ...
          alert(`Error archiving resource: ${errorMessage}`);
        }
      });
    }
  }
  ...
  onUnarchiveResource(resource: Resource): void {
    ...
    if (confirm(`Are you sure you want to unarchive this resource posting? It will be restored to active status and visible to other users.`)) {
      this.isLoading = true;
      this.apiService.unarchiveBenchResource(resource.id).subscribe({
        next: () => {
        ...
        error: (error) => {
          ...
          alert(`Error unarchiving resource: ${errorMessage}`);
        }
      });
    }
  }
  ...
  onDeleteResource(resource: Resource): void {
    ...
    if (confirm(`Are you sure you want to delete this resource posting? This will also delete all related interest submissions.`)) {
      this.isLoading = true;
