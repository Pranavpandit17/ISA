...
export class DashboardComponent implements OnInit {
      ...
      next: (response: any) => {
        ...
        const publishedEvents = events.filter((e: any) => {
          ...
          if (e.endDate) {
            ...
            endDate.setHours(0, 0, 0, 0);
            if (endDate < now) {
              return false; // Event is closed, don't show to members
            }
          } else if (e.startDate) {
        ...
        this.allEvents = publishedEvents.map((e: any) => ({
          ...
          availableSeats: e.availableSeats !== undefined ? e.availableSeats : null
        }));
  ...
  onEventPaymentRequest(paymentData: any): void {
    ...
    if (paymentData && paymentData.event) {
      this.selectedEvent = paymentData.event;
      // Set quantity if provided
      if (paymentData.quantity) {
        this.selectedEvent.quantity = paymentData.quantity;
      }
